    ...
    <section className=" container pb-8 ">
      <nav className="flex items-center justify-between py-8">
        ...
        <div className="flex gap-10 items-center">
          ...
          <Button variant={"outline"} className="p-8 text-lg rounded-xl border-2 border-primary">
            Book a consultation
          </Button>
        </div>
      </nav>
      <div className="grid grid-cols-2 gap-20 my-8">
        <div className="max-w-lg space-y-10">
          ...
          <Button className="p-8 text-lg rounded-xl">Book a consultation</Button>
        </div>
      ...
      <div className="flex justify-between items-center mt-20">
        {BRANDS.map((brand) => (
          <Image src={`/${brand}.svg`} className="h-12 object-fill" alt="brand" width={125} height={48} key={brand} />
        ))}
      </div>
    </section>
  );
